...
function createPublicApi(registry, { onRegister } = {}) {
  return {
    ...
    register(node) {
      ...
      const wrapped = Object.assign({ origin: 'api', priority: DEFAULT_PRIORITY, slot: 'early' }, node);
      if (wrapped.slot !== 'early' && wrapped.slot !== 'late') {
        throw new TypeError('slot must be "early" or "late"');
      }
