...
const KNOWN_TOP_KEYS = ['enable', 'debug', 'strict', 'inject_css', 'inject_js', 'presets', 'hooks', 'tap'];
const KNOWN_TAP_KEYS = ['enable', 'match', 'dir'];
const KNOWN_HOOK_KEYS = ['command', 'script', 'stage', 'priority', 'name', 'timeout', 'enable', 'match'];
...
function checkNodes(nodes) {
  ...
  for (const node of nodes) {
    const key = node.stage + '@' + (Number.isFinite(node.priority) ? node.priority : 10);
...
module.exports = { checkPluginConfig, checkNodes };
