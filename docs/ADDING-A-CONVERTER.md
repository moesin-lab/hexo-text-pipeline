...
Follow the steps in order; nothing under `lib/core/` needs to change.
...
3. **Pick a stage**:
   - The rewrite result is still markdown (e.g. `==highlight==` → `<mark>`, `%%comment%%` → removed) → `before_post_render`; inline replacement MUST be wrapped with `replaceOutsideCode` from `core/markdown-guard` to avoid rewriting code blocks.
   - The conversion depends on rendered HTML structure (e.g. callout builds on `<blockquote>`) → `after_post_render`; no guard needed.
4. **Implement**: parsing goes in `parse.js`, output in `render.js` (a simple syntax can keep everything in `index.js`). Use `core/post-index` if you need the post index. For default styles, add `styles.js` and expose it via the `css` field in `index.js`.
5. **Register**: add one `require('./<name>')` line in `lib/converters/registry.js`.
6. **Verify**: `npm test` all green.
7. **Document**: update the supported-syntax table in both READMEs (English and Chinese).
