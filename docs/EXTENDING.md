...
## Level 1: a user hook in your site repo (the default answer)

Site-specific transform → no code in this repo at all:
...
4. **Implement**: parsing in `parse.js`, output in `render.js` (simple syntax can stay in `index.js`). Default styles via the `css` field, frontend loader via `js`. If most renderers already handle it, ship `enabledByDefault: false`.
