...
|-------|---------|------|------------------------|
| `before_post_render` | 文章的**原始 markdown**，frontmatter 已剥离。wiki 链接、`%%注释%%`、围栏代码块都还在——本插件的 stage filter 以优先级 5 注册，抢在 Hexo 内置代码块处理之前 | 每篇文章/页面一次 | 完整 post 对象：`title`、`source`（`_posts/x.md`）、`path`、`slug`、frontmatter 字段 |
| `after_post_render` | 文章**渲染后的 HTML 片段**（无布局）。代码块已是 `<figure class="highlight">…` | 每篇文章/页面一次 | 同上 |
| `after_render:html` | **完整页面 HTML**，含布局、`<head>`、注入的资源 | 每个生成页面一次（文章、首页、归档…） | 输出文件的 `{ path }` |
| `after_render:css` / `:js` | 生成资源的完整文本 | 每个资源一次 | `{ path }` |

`before_post_render` 的输入实例（tap 抓取的真实样本）：
...
别靠猜——开一次 tap，读你目标 stage 的 `00-input.txt`（见文末）。
...
| `priority` | 否 | `10` | 小者先跑；同级按声明顺序（preset node 先于 hook 加载） |
...
├── 00-input.txt          ← 你的 hook 收到的就是这个（前面的 node 已跑过）
├── 01-<node>.txt         ← 每个改动了文本的 node 改完后的样子
└── …                     ← 最后一个文件 = 该 stage 的最终输出
```
